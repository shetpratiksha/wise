...
(function () {
  ...
  function floatThead($timeout, $log) {
    ...
    function link($scope, $element, $attrs, ngModel) {
      $scope.$watch('floatTheadEnabled', function (newVal) {
        if (newVal === true) {
          if (angular.isObject($scope.floatThead)) {
            jQuery($element).find('table').floatThead({
          ...
          jQuery($element).find('table').floatThead('destroy');
        }
      });
      ...
      if (ngModel) {
        ...
        ngModel.$formatters.push(function () {
          ...
          if ($scope.floatTheadEnabled && angular.isObject($scope.floatThead)) {
            ...
            $timeout(function() {
              jQuery($element).find('table').floatThead('reflow');
            });
          }
        });
      } else {
      ...
      $element.find('table').bind('update', function () {
        $timeout(function () {
          jQuery($element).find('table').floatThead('reflow');
        }, 0);
      });

      $element.find('table').bind('$destroy', function () {
        jQuery($element).find('table').floatThead('destroy');
      });
    }
  }
